...
#
# Inputs / Expectations:
# - Prompts interactively for MAC address, line number, caller name, and email.
# - Uses /var/lib/tftpboot template (SEP0000000.cnf.xml).
# - Requires write access to TFTP boot directory and Asterisk config files.
# - Requires Asterisk CLI access to reload SIP, dialplan, and voicemail.
#
# Behavior:
# - Copies and updates the template with the provided MAC and extension number.
# - Appends SIP peer, dialplan, and voicemail entries to sip.conf, extensions.conf, and voicemail.conf.
